# scripts/upload_nh_institution_images.py
"""
New Hampshire-focused script to upload images to institutions
"""
import sys
import os
...
def list_nh_institutions():
    """Show all New Hampshire institutions with their IDs and names"""
    ...
    try:
        ...
        print(f"Found {len(institutions)} New Hampshire institutions:")
        print("=" * 80)
...
def show_nh_institutions_needing_images():
    """Show NH institutions that need images"""
    engine = create_engine(str(settings.DATABASE_URL))
    SessionLocal = sessionmaker(bind=engine)
    db = SessionLocal()
    ...
    try:
        ...
        if institutions:
            print(f"Found {len(institutions)} NH institutions needing images:")
            print("=" * 80)
            for inst in institutions:
                print(f"ID: {inst.id:3d} | {inst.name}")
                print(f"      | {inst.city}, NH")
            ...
            print("All NH institutions have images! 🎉")

    finally:
        db.close()


def show_nh_institutions_with_poor_images():
    """Show NH institutions with low quality images that need replacement"""
    engine = create_engine(str(settings.DATABASE_URL))
    SessionLocal = sessionmaker(bind=engine)
    db = SessionLocal()
    ...
    try:
        ...
        if institutions:
            print(
                f"Found {len(institutions)} NH institutions with poor quality images:"
            )
            print("=" * 80)
            for inst in institutions:
                print(f"ID: {inst.id:3d} | {inst.name}")
            ...
            print("All NH institutions have good quality images! 🎉")

    finally:
        db.close()


def upload_image_to_nh_institution(
    institution_id: int, image_filename: str, quality_score: int = 85
):
    """Upload an image to a specific NH institution"""
    engine = create_engine(str(settings.DATABASE_URL))
    SessionLocal = sessionmaker(bind=engine)
    db = SessionLocal()
    ...
    try:
        institution = (
            ...
            .filter(Institution.state == "NH")  # Ensure it's a NH institution
            .first()
        )

        if not institution:
            print(f"New Hampshire institution with ID {institution_id} not found")
            return False
        ...
        possible_paths = [
            ...
            f"../manual_images/{image_filename}",
            f"nh_images/{image_filename}",  # New NH-specific folder
            f"../nh_images/{image_filename}",
        ]
        ...
        print(f"Uploading to NH institution: {institution.name}")
        ...
        with Image.open(image_path) as img:
            ...
            cdn_filename = f"nh_institution_{institution_id}_{base_name}_optimized.jpg"

            cdn_url = uploader.upload_image(
                image_bytes=img_bytes.getvalue(),
                file_path=f"institutions/nh/{cdn_filename}",  # NH-specific folder
...
def batch_upload_nh_images():
    """Batch upload images for NH institutions based on naming convention"""
    print("Batch upload for NH institutions")
    ...
    image_dirs = [".", "manual_images", "../manual_images", "nh_images", "../nh_images"]
    ...
    for inst_id, image_path in found_images:
        ...
        if upload_image_to_nh_institution(inst_id, image_path, 85):
            successful += 1
        else:
            failed += 1
...
def main():
    """Main menu for NH institution image management"""
    print("🏫 New Hampshire Institution Image Manager")
    print("=" * 50)

    while True:
        print("\nWhat would you like to do?")
        print("1. List all NH institutions")
        print("2. Show NH institutions needing images")
        print("3. Show NH institutions with poor quality images")
        print("4. Upload image to specific NH institution")
        print("5. Batch upload NH images")
        print("6. Exit")

        choice = input("\nEnter choice (1-6): ").strip()

        if choice == "1":
            list_nh_institutions()

        elif choice == "2":
            show_nh_institutions_needing_images()

        elif choice == "3":
            show_nh_institutions_with_poor_images()

        elif choice == "4":
            try:
                institution_id = int(input("Enter NH institution ID: ").strip())
                ...
                if upload_image_to_nh_institution(
